...
import { propertyModel, buyerModel, ContactModel, usermodModel } from '../model/table.js'
    ...
    try {
        ...
        const finalData = await Promise.all(
            raw?.map(async (item) => {
                ...
                const userData = await usermodModel.findOne({ _id: item?.userId });
                return {
                    _id: item?._id,
...
adminRoute.get('/admin-user-list', async (req, res) => {
    try {
        const result = await usermodModel.find({ userType: "user" });
        if (result?.length > 0) {
            res.json({
                code: 200,
                message: "Data fetched successfully..",
